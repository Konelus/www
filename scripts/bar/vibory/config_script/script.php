...
define host{
    ...
    contact_groups '.$sys_var[$count]['contact_groups'].';
            ...
            {
                $fp = fopen(dirname(__FILE__).'/config/'.str_replace(',', '_', str_replace('/', '-', $vibory_var[$count]['description'])).".cfg", "w");
                fwrite($fp, $str);
                fclose($fp);

                $fp = fopen(dirname(__FILE__).'/config/1251/'.str_replace(',', '_', str_replace('/', '-', $vibory_var[$count]['description'])).".cfg", "w");
                fwrite($fp, $win_str);
                fclose($fp);
            }
    }
...
members '.$host_16.';
        ...
        //if ($vibory_var[$count]['description'] != null)
        //{
            $fp = fopen(dirname(__FILE__).'/config/new/UTF-8.cfg', "w");
            fwrite($fp, $str_2);
            fclose($fp);

            $fp = fopen(dirname(__FILE__).'/config/new/1251.cfg', "w");
            fwrite($fp, $win_str_2);
            fclose($fp);
        //}
...
<div>
    <form method = 'post'>
        <input style = 'border: 0; background: white; color: navy; cursor: pointer;' type = 'submit' name = 'create' value = 'Создать конфиги VIBORY'>
    </form>
</div>