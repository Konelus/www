...
<?php
    ...
    if (isset ($_POST['make_schedule']))
    {
        ...
        $fp = fopen($_SERVER['DOCUMENT_ROOT']."/scripts/schedule_ege/config/raspisanie.cfg", "w");
        fwrite($fp, $fstr);
        fclose($fp);

        $fp = fopen($_SERVER['DOCUMENT_ROOT']."/scripts/schedule_ege/config/raspisanie_1251.cfg", "w");
        fwrite($fp, $win_str);
        fclose($fp);
    }
