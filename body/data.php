<?php
    ...
    {
        $searched_tr = 0;
        $SQL_QUERY_select_data = $mysqli->query("select * from `".$podcat_name[1]."`".$sort." LIMIT  ".$lim." ");
...
//        $max_count = mysqli_num_rows($SQL_QUERY_select_data);
        if (isset ($_POST['inversion']))
        { $SQL_QUERY_select_data = $mysqli->query("select * from `".$podcat_name[1]."` where `".$searched_td ."` !=  '".trim($caption)."' ".$sort." LIMIT  ".$lim." "); }
        else if (!isset ($_POST['inversion']))
        { $SQL_QUERY_select_data = $mysqli->query("select * from `".$podcat_name[1]."` where `".$searched_td ."` LIKE  '%".trim($caption)."%' ".$sort." LIMIT  ".$lim." "); }
        if ($SQL_QUERY_select_data != null)
        { $max_count = mysqli_num_rows($SQL_QUERY_select_data); }
