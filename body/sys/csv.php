<?php
    ...
    else
    {
        ...
        if (stripos("{$_POST['value']}","--"))
        {
            list($date, $temp) = explode(" ","{$_POST['value']}");
            list($first, $second) = explode("--","{$date}");
            list($date_1, $month_1, $year_1) = explode(".","{$first}");
            list($date_2, $month_2, $year_2) = explode(".","{$second}");

            if ($month_1 == $month_2)
            {
                for ($date_1; $date_1 <= $date_2; $date_1++)
                { $val .= "`{$show_flip[$_POST['table']]}` LIKE '%{$date_1}.{$month_1}.{$year_1}%' OR"; }
            }
        }
        else
        { $val = "`{$show_flip[$_POST['table']]}` LIKE '%{$_POST['value']}%' OR "; }
        $DB->select("*","{$substring}", "{$val} `id` = '1' OR `id` = '2'");
    }
